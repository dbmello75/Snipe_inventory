...
CURRENT_CPU=$(echo "$CURRENT" | jq -r '
    .custom_fields
    | to_entries[]
    | select(.value.field == "_snipeit_cpu_2")
    | .value.value // ""
')

CURRENT_RAM=$(echo "$CURRENT" | jq -r '
    .custom_fields
    | to_entries[]
    | select(.value.field == "_snipeit_ram_total_6")
    | .value.value // ""
')

CURRENT_MEMORY=$(echo "$CURRENT" | jq -r '
    .custom_fields
    | to_entries[]
    | select(.value.field == "_snipeit_memory_details_7")
    | .value.value // ""
')

CURRENT_STORAGE=$(echo "$CURRENT" | jq -r '
    .custom_fields
    | to_entries[]
    | select(.value.field == "_snipeit_storage_3")
    | .value.value // ""
')

CURRENT_UUID=$(echo "$CURRENT" | jq -r '
    .custom_fields
    | to_entries[]
...
compare_field "Storage" "$CURRENT_STORAGE" "$STORAGE"
